...
var init_utils = __esm({
  "node_modules/linkedom/esm/shared/utils.js"() {
    ...
    knownAdjacent = (prev, next2) => {
      prev[NEXT] = next2;
      next2[PREV] = prev;
    };
    knownBoundaries = (prev, current, next2) => {
      knownAdjacent(prev, current);
      knownAdjacent(getEnd(current), next2);
    };
    knownSegment = (prev, start, end, next2) => {
      knownAdjacent(prev, start);
      knownAdjacent(getEnd(end), next2);
    };
    knownSiblings = (prev, current, next2) => {
      knownAdjacent(prev, current);
...
var init_jsdon = __esm({
  "node_modules/linkedom/esm/shared/jsdon.js"() {
    ...
    init_symbols();
    init_utils();
    loopSegment = ({ [NEXT]: next2, [END]: end }, json) => {
      while (next2 !== end) {
  ...
  loop: while (selectorIndex < selector.length) {
    ...
    switch (firstChar) {
      ...
      case CharCode.LeftSquareBracket: {
        ...
        stripWhitespace(0);
        let action2 = AttributeAction.Exists;
        ...
        if (possibleAction) {
          ...
          action2 = AttributeAction.Equals;
          stripWhitespace(1);
        }
        let value = "";
...
function renderTag2(element, options, xmlMode) {
  ...
  const isVoid3 = !xmlMode && voidElements2.has(name);
  let tag = `<${name}${formatAttributes2(element.attribs, options, xmlMode)}`;
  if (children.length === 0 && (xmlMode ? options.selfClosingTags !== false : options.selfClosingTags && isVoid3)) {
    tag += xmlMode ? "/>" : " />";
  } else {
    tag += ">";
    ...
    attributeRules = {
      ...
      hyphen(next2, data, options) {
        ...
        if (shouldIgnoreCase(data, options)) {
          ...
          return function hyphenIC(element) {
            const attribute2 = adapter2.getAttributeValue(element, name);
            return attribute2 != null && (attribute2.length === length || attribute2.charAt(length) === "-") && attribute2.substr(0, length).toLowerCase() === value && next2(element);
          };
        }
        return function hyphen(element) {
          const attribute2 = adapter2.getAttributeValue(element, name);
          return attribute2 != null && (attribute2.length === length || attribute2.charAt(length) === "-") && attribute2.substr(0, length) === value && next2(element);
        };
      },
      ...
      start(next2, data, options) {
        ...
        if (shouldIgnoreCase(data, options)) {
          ...
          return (element) => {
            const attribute2 = adapter2.getAttributeValue(element, name);
            return attribute2 != null && attribute2.length >= length && attribute2.substr(0, length).toLowerCase() === value && next2(element);
          };
        }
        ...
        if (shouldIgnoreCase(data, options)) {
          value = value.toLowerCase();
          return (element) => adapter2.getAttributeValue(element, name)?.substr(length).toLowerCase() === value && next2(element);
        }
...
function compilePseudoSelector(next2, selector, options, context, compileToken2) {
  ...
  if (typeof userPseudo === "function") {
    ...
    return (element) => userPseudo(element, data) && next2(element);
  }
  if (name in filters) {
    return filters[name](next2, data, options, context, compileToken2);
  }
  if (name in pseudos) {
    const pseudo = pseudos[name];
    verifyPseudoArguments(pseudo, name, data, 2);
    return (element) => pseudo(element, options, data) && next2(element);
  }
...
function convertOptionFormats(options) {
  ...
  finalOptions.adapter ?? (finalOptions.adapter = defaultOptions.adapter);
  finalOptions.equals ?? (finalOptions.equals = finalOptions.adapter?.equals ?? defaultEquals);
  return finalOptions;
}
...
// src/dom-shim.ts
var require_dom_shim = __commonJS({
  "src/dom-shim.ts"() {
    "use strict";
    ...
    var windowRef = globalWithDOM.window;
    if (!windowRef.document) {
      const { document: document2 } = parseHTML("<html><body></body></html>");
      windowRef.document = document2;
    }
...
var import_dom_shim = __toESM(require_dom_shim());
...
var voidElements4 = ["AREA", "BASE", "BR", "COL", "COMMAND", "EMBED", "HR", "IMG", "INPUT", "KEYGEN", "LINK", "META", "PARAM", "SOURCE", "TRACK", "WBR"];
function isVoid2(node) {
  return is3(node, voidElements4);
}
function hasVoid(node) {
  return has(node, voidElements4);
}
...
function escapeMarkdown(string) {
  return markdownEscapes.reduce(function(accumulator, escape4) {
...
function filterValue(rule, node, options) {
  ...
  if (typeof filter3 === "string") {
    if (filter3 === node.nodeName.toLowerCase()) return true;
  } else if (Array.isArray(filter3)) {
    if (filter3.indexOf(node.nodeName.toLowerCase()) > -1) return true;
  } else if (typeof filter3 === "function") {
    if (filter3.call(rule, node, options)) return true;
  } else {
...
// src/main.ts
var headingStyle = (options) => options.headingStyle === "setext" ? "setext" : "atx";
var bulletListMarker = (options) => options.bulletListMarker === "-" || options.bulletListMarker === "+" ? options.bulletListMarker : "*";
var emDelimiter = (options) => options.emDelimiter === "_" ? "_" : "*";
var strongDelimiter = (options) => options.strongDelimiter === "__" ? "__" : "**";
var linkStyle = (options) => options.linkStyle === "referenced" ? "referenced" : "inlined";
var toMarkdown = (html, options) => {
...
var action = (selection, options) => {
  const html = openclip.input.html;
  const text = openclip.input.text;
  const markdown = html && html.trim().length > 0 ? toMarkdown(html, options) : text;
